...
class PuzzleGame:
    ...
    def get_state(self):
        # Return a 2D list representation the puzzle
        state = []
...
def run():
    ...
    while True:
        ...
        # If a solution has been found, animate the solution steps
        # else, search for the solution
        if solution is not None:
            # If solution has been fully shown
            if solution_index == len(solution):
                continue

            # Slide the tile according to the solution step
            ...
            # Draw the updated puzzle state
            puzzle_game.draw(surface)

            # Display 'Solved' text if solution has been fully shown
            if solution_index == len(solution):
