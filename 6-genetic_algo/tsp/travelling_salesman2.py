...
class Population:
    ...
    def generate_members(self, mutation_prob):
        ...
        parents = self._select_parents(sorted_members, self.size - ELITISM)
        new_members = []
        for i in range(self.size - ELITISM):
            # parent = self._select_parent(sorted_members)
            parent = parents[i]
    ...
    def _select_parents(self, sorted_members, n):
        size = len(sorted_members)
        ranks_sum = int((size + 1) * (size / 2))
        probs = [x / ranks_sum for x in range(1, size + 1)]
        parents = random.choices(sorted_members, weights=probs, k=n)

        return parents

    def _select_parent(self, sorted_members):
        size = len(sorted_members)
        ranks_sum = int((size + 1) * (size / 2))
        rand = random.random()
        counter = 0

        for rank, parent in enumerate(sorted_members, start=1):
            counter += rank / ranks_sum
            if counter > rand:
                return parent
...
if __name__ == '__main__':
    experiment_path = 'nigeria_rank_selection2_30k'
    ...
    for mutation_prob in [0.01, 0.05, 0.1, 0.15, 0.2]:
    # for mutation_prob in [0.005, 0.008, 0.01, 0.015]:
